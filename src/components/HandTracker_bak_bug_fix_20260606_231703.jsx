...
const SEQ_TOUCH_RATIO = 0.20  // 시퀀스 잠금용 핀치 임계값
const SEQ_TIMEOUT     = 60    // 탭 간 최대 허용 프레임 (~2초)
const UNLOCK_ORDER    = [8, 12, 16, 20]   // 검지→중지→약지→소지
const LOCK_ORDER      = [20, 16, 12, 8]   // 소지→약지→중지→검지
...
export default function HandTracker() {
  ...
  useEffect(() => {
    ...
    // 손별 잠금 시퀀스 상태
    const seqState = {
      Left:  { step: 0, dir: null, lastTouchFinger: null, wasTouch: false, lastStepFrame: 0 },
      Right: { step: 0, dir: null, lastTouchFinger: null, wasTouch: false, lastStepFrame: 0 },
    }
    ...
    function detect() {
      ...
      if (lms.length === 0) {
        ...
        for (const s of ['Left', 'Right']) { seqState[s].wasTouch = false; seqState[s].lastTouchFinger = null }
        resetHandState()
        rafId = requestAnimationFrame(detect)
        return
      }
      ...
      // ── 양손 엄지+검지 핀치 → 줌 모드 ──
      if (bothZoomPinch) {
        lms.forEach(lm => drawHand(lm, ctx, W, H, true, isDark))
        const p0 = drawPinchDot(lms[0], 4, 8, ctx, W, H, isDark)
        const p1 = drawPinchDot(lms[1], 4, 8, ctx, W, H, isDark)
        ...
        if (inIndexMode) {
          lms.forEach((lm, i) => drawHand(lm, ctx, W, H, i === 0, isDark))
          ...
          lms.forEach((lm, i) => drawHand(lm, ctx, W, H, scrollInfos[i].activePinch || backInfos[i].activePinch, isDark))
          scrollInfos.forEach((info, i) => {
            if (info.activePinch) drawPinchDot(lms[i], 4, 12, ctx, W, H, isDark)
          })
          backInfos.forEach((info, i) => {
            if (info.activePinch) drawPinchDot(lms[i], 4, 8, ctx, W, H, isDark)
          })
      ...
      for (let hi = 0; hi < lms.length; hi++) {
        ...
        // 엄지와 가장 가까운 손가락 끝 탐색
        let activeFinger = null, minDist = Infinity
        for (const tip of [8, 12, 16, 20]) {
          const d = Math.hypot(lm[4].x - lm[tip].x, lm[4].y - lm[tip].y)
          if (handSize > 0 && d / handSize < SEQ_TOUCH_RATIO && d < minDist) {
            minDist = d; activeFinger = tip
          }
        }

        // 접촉 중인 경우 핀치 점 시각화
        if (activeFinger !== null) drawPinchDot(lm, 4, activeFinger, ctx, W, H, isDark)

        // 하강 엣지(손가락 뗀 순간)에 시퀀스 처리
        if (activeFinger === null && ss.wasTouch && ss.lastTouchFinger !== null) {
          const f = ss.lastTouchFinger
          if (ss.step === 0) {
            if (f === UNLOCK_ORDER[0])    { ss.dir = 'unlock'; ss.step = 1; ss.lastStepFrame = frameCount }
            else if (f === LOCK_ORDER[0]) { ss.dir = 'lock';   ss.step = 1; ss.lastStepFrame = frameCount }
          } else if (frameCount - ss.lastStepFrame > SEQ_TIMEOUT) {
            ss.step = 0; ss.dir = null
          } else {
            const order = ss.dir === 'unlock' ? UNLOCK_ORDER : LOCK_ORDER
            if (f === order[ss.step]) {
              ss.step++; ss.lastStepFrame = frameCount
              if (ss.step === 4) {
                handState[side === 'Left' ? 'leftLocked' : 'rightLocked'] = ss.dir !== 'unlock'
                ss.step = 0; ss.dir = null
              }
            } else {
              ss.step = 0; ss.dir = null
          ...
          ss.lastTouchFinger = null
        }

        if (activeFinger !== null) ss.lastTouchFinger = activeFinger
        ss.wasTouch = activeFinger !== null
      }
