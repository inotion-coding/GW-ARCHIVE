...
const FIST_BACK_FRAMES  = 90    // 손등 주먹 유지 필요 프레임 (~3초 at 30fps)
...
// 잠금 제스처 진행 호 — progress 0~1, futureLocked true=잠김(red) false=해제(green)
function drawHoldProgress(lm, ctx, W, H, progress, futureLocked) {
  const wx = (1 - lm[0].x) * W
  const wy = lm[0].y * H
  const r  = 34
  ctx.strokeStyle = futureLocked ? 'rgba(220,70,70,0.8)' : 'rgba(80,220,120,0.8)'
  ctx.lineWidth   = 3
  ctx.setLineDash([])
  ctx.beginPath()
  ctx.arc(wx, wy, r, -Math.PI / 2, -Math.PI / 2 + progress * 2 * Math.PI)
  ctx.stroke()
}
    ...
    function detect() {
      ...
      for (let hi = 0; hi < lms.length; hi++) {
        ...
        const isFist    = lm[8].y  > lm[6].y
                       ...
                       && lm[16].y > lm[14].y
                       && lm[20].y > lm[18].y
        const isGesture = isFist && isBackFacing(lm, side)

        if (isGesture && ls.flashFrames === 0) {
          ls.holdFrames++
          // 진행 호: futureLocked = !handState[key] (현재 잠김→해제=green, 해제→잠김=red)
          if (ls.holdFrames < FIST_BACK_FRAMES) {
            drawHoldProgress(lm, ctx, W, H, ls.holdFrames / FIST_BACK_FRAMES, !handState[key])
          }
          if (ls.holdFrames >= FIST_BACK_FRAMES) {
