...
export default function HandTracker() {
  ...
  useEffect(() => {
    ...
    let lastIndexY   = null
    let tapFired     = false
    let fistFrames   = 0          // 주먹 지속 프레임 수
    const FIST_HOLD  = 18         // 약 0.6초 유지 시 뒤로 가기 발동
    ...
    function detect() {
      ...
      const isFistNow   = firstLmFist[8].y  > firstLmFist[6].y   // 검지 접힘
                       && firstLmFist[12].y > firstLmFist[10].y  // 중지 접힘
                       && firstLmFist[16].y > firstLmFist[14].y  // 약지 접힘
                       && firstLmFist[20].y > firstLmFist[18].y  // 소지 접힘
      if (isFistNow) {
        fistFrames++
      ...
      if (lms.length === 2 && zoomInfos[0].activePinch && zoomInfos[1].activePinch) {
        ...
        const inIndexMode = scrollActive < 0 && isIndexOnly(firstLmFist)
