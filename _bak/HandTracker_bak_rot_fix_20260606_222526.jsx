...
export default function HandTracker() {
  ...
  useEffect(() => {
    ...
    const rollState = {
      Left:  { lastAngle: null, velEma: 0, cooldown: 0 },
      Right: { lastAngle: null, velEma: 0, cooldown: 0 },
    }
    ...
    function detect() {
      ...
      const anyZoomPinch   = zoomInfos.some(z => z.activePinch)
      const bothZoomPinch  = lms.length === 2 && zoomInfos[0].activePinch && zoomInfos[1].activePinch

      // 쿨다운 카운트다운
      for (const side of ['Left', 'Right']) {
        if (rollState[side].cooldown > 0) rollState[side].cooldown--
      }

      // 핀치(스크롤/줌 모두) 또는 마우스·터치 드래그 중에는 회전 차단
      if (!anyScrollPinch && !anyZoomPinch && !handState.dragging) {
        for (let hi = 0; hi < lms.length; hi++) {
          ...
          if (rs.lastAngle !== null && rs.cooldown === 0) {
            ...
            rs.velEma = rs.velEma * (1 - ROT_EMA_K) + dAngle * ROT_EMA_K

            // 빠른 회전 + 손등이 카메라를 향할 때만 임펄스 발동
            if (Math.abs(rs.velEma) > ROT_THRESH && isBackFacing(lm, side)) {
              const dir = side === 'Right' ? 1 : -1
              handState.rotDx = dir * ROT_IMPULSE   // 캐러셀이 소비 후 0으로 리셋
              rs.cooldown     = ROT_COOLDOWN
              rs.velEma       = 0
              drawRotationArc(lm, ctx, W, H, side)
            }
          } else if (rs.cooldown === 0) {
            rs.lastAngle = angle  // 쿨다운 중에는 각도 추적 중지
          }

          if (rs.cooldown === 0) rs.lastAngle = angle
        }
      } else {
        // 핀치/줌 중에는 roll 히스토리 리셋
        for (const side of ['Left', 'Right']) {
          rollState[side].lastAngle = null
          rollState[side].velEma   = 0
        }
      }
