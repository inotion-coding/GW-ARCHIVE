...
export default function Carousel() {
  ...
  useEffect(() => {
    ...
    const onDown = e => {
      ...
      dragStartX = px(e); dragMoved = false
    }
    const onMove = e => {
      if (!drag) return
      const x = px(e)
    ...
    const onUp = e => {
      ...
      if (!dragMoved) {
        ...
        const centerCard = CARDS[((base % N) + N) % N]
        navigateRef.current(`/motion/${centerCard.id}`)
      }
    }
    ...
    function tick() {
      ...
      if (handState.active) {
        ...
        if (handState.rotDx !== 0) {
          // 손 회전: 관성 없이 target 직접 이동, snap 중력은 회전 중 비활성
          target  += handState.rotDx
          handVel  = handState.rotDx   // 놓는 순간 관성 자연 연결
      ...
      if (!drag && !handState.activePinch && handState.rotDx === 0) {
