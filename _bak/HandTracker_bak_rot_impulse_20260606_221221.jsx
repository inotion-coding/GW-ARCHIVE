...
// 회전 감지 파라미터
const ROT_EMA_K    = 0.35   // 각속도 EMA 계수 (높을수록 반응 빠름)
const ROT_THRESH   = 0.022  // rad/frame — 이 이상이면 회전 중 판정
const ROT_SPEED    = 0.14   // 회전 중 프레임당 스크롤 속도 (카드 단위)
...
export default function HandTracker() {
  ...
  useEffect(() => {
    ...
    const rollState = {
      Left:  { lastAngle: null, velEma: 0 },
      Right: { lastAngle: null, velEma: 0 },
    }
    ...
    function detect() {
      ...
      handState.rotDx = 0  // 매 프레임 초기화 후 회전 감지 시 설정

      if (!anyScrollPinch && !bothZoomPinch) {
        for (let hi = 0; hi < lms.length; hi++) {
          const lm   = lms[hi]
          const side = handedness[hi]?.[0]?.categoryName  // 'Left' | 'Right'
          ...
          if (rs.lastAngle !== null) {
            ...
            // 회전 임계값 초과 시 방향에 따라 스크롤
            if (Math.abs(rs.velEma) > ROT_THRESH) {
              // 오른손 → 오른쪽(+), 왼손 → 왼쪽(−)
              const dir = side === 'Right' ? 1 : -1
              handState.rotDx += dir * ROT_SPEED
